...
import org.eclipse.om2m.ipu.mqtt.MqttProxyMonitor;
...
public class ProtocolProcessor {
	...
	public void internalPublish(PublishMessage msg) {
		...
		// TODO 唐君豪 junhao
		// m_interceptor.notifyTopicPublished(msg, "BROKER_SELF");
	}
	...
	void route2Subscribers(IMessagesStore.StoredMessage pubMsg) {
		...
		for (final Subscription sub : subscriptions.matches(topic)) {
			...
			LOG.debug("Broker republishing to client <{}> topic <{}> qos <{}>, active {}", sub.getClientId(), sub.getTopicFilter(), qos, targetSession.isActive());
		...
		// INFO 唐君豪 junhao
		// 如果topic包含om2m，則認為是要送給om2m的。
		if (topic.contains("om2m") || topic.contains("nscl") || topic.contains("gscl")) {
			new Thread() {
				public void run() {
					String msg = DebugUtils.payload2Str(origMessage);
					MqttProxyMonitor.sendToOM2M(topic, msg);
				}
			}.start();
		}
	}
