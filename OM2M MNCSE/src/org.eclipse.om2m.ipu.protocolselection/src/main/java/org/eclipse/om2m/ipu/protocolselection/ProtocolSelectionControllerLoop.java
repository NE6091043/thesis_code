...
public class ProtocolSelectionControllerLoop implements IpuService {
	/** Logger */
	private static Log LOGGER = LogFactory.getLog(ProtocolSelectionControllerLoop.class);
	...
	public ProtocolSelectionControllerLoop() {
	...
	public ResponseConfirm doExecute(RequestIndication requestIndication) {
		...
		try {
			...
			} else if (info[info.length - 5].equals("process")) {
				if (info[info.length - 4].equals("start")) {

					final String strPacketLossRate = info[info.length - 3];
					final String strDataSize = info[info.length - 2];
					final String strLoopNum = info[info.length - 1];

					new Thread() {
						public void run() {
							try {
								writePacketLossRateTaskLoop task = new writePacketLossRateTaskLoop(strPacketLossRate,strDataSize,strLoopNum);
								task.run();
							} catch (Exception e) {
								e.printStackTrace();
							}
