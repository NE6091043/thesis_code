            ...
            public void run() {
                for(;;) {
                    if(Redirector.flag==true) {
                        ...
                        try {
                            state_action.decision(Redirector.cnt);
...
//      StartHTTPServer();
    ...
    public static void tshark() throws IOException {
        ProcessBuilder pb = new ProcessBuilder("tshark", "-l", "-i", "ens38", "-f", allcmd);
        Process process = pb.start();
        
        // String url = "http://140.116.247.69:9000/receive_throughput";
